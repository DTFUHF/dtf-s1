...
void frsky_d8_rx_bind_init(void)
{
  ...
  CC2500_SetChan(D8_FRSKY_BIND_CHANNEL);
...
uint8_t frsky_d8_rx_binding(void)
{
  ...
  if (HAL_GPIO_ReadPin(CC2500_GD0_GPIO, CC2500_GD0_PIN) == GPIO_PIN_SET)
  {
    ...
    if (rx_packet_length && (rx_packet_length < D8_MAX_PACKET_SIZE))
    {
      CC2500_ReadRXData(rxmsg.packet, rx_packet_length);
      
      if (binding_state == 0)
      {
        if ((rxmsg.d8_bind_packet_data.header1 == 0x0103) && (rxmsg.packet[5] == 0x00) && (rxmsg.packet[19] & 0xA0)) // 0xA0 seems to be some mask, in lieu of crc decoding
        {
          m_config.frsky_d8_bind_info.bind_id = rxmsg.d8_bind_packet_data.header2;
        ...
        if ((rxmsg.d8_bind_packet_data.header2 == m_config.frsky_d8_bind_info.bind_id) && (rxmsg.packet[5] == binding_state) && (rxmsg.packet[19] & 0xA0)) // binding address check
        {
          for (uint8_t i=0; i<5; i++)
          {
            hopChannelsTemp[binding_state+i] = rxmsg.packet[6+i];
            
            if ((binding_state + i) == (D8_NUM_HOP_CHANNELS - 1)) // end condition for d8
            {
              binding_state = -1;
              binding_loops++;
              if (binding_loops == 1)
              {
                for( uint8_t i = 0; i < D8_NUM_HOP_CHANNELS; i++ ) // copy temp array to working array for future compare
                {
                  m_config.frsky_d8_bind_info.hopChannels[i] = hopChannelsTemp[i];
                }
              }
              break;
            }
  ...
  else if ((binding_loops > 1) & (binding_state == -1))
  {
    ...
    {
      if(hopChannelsTemp[i] != m_config.frsky_d8_bind_info.hopChannels[i])
      {
        binding_loops = 0;
